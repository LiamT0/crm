...
async function requireEmployeeAuth() {
  ...
  // MVP: don't hard-block on roles yet.
  // Netlify Identity role assignment typically needs admin tooling; until then,
  // we allow any authenticated user into the internal portal and just surface a
  // warning in the console. We'll add strict role gating later.
  if (!userHasRole(user, REQUIRED_ROLE)) {
    console.warn('Logged in user does not have the employee role. Access allowed (MVP).');
  }
